...
Values are not case-sensitive and surrounding spaces are ignored, so `Off` and `off` are the same. A value the relay does not recognise is logged and ignored, and the default applies.
...
- `GET /api/stats` — relay stats for the admin page. Returns `{ version, latestVersion, updateAvailable, claimed, ownerPubkey, relayPubkey, totalEvents, events24h, ingested24h, rowsWrittenToday, storageBytes, storageBytesLimit, dailyRowsWrittenLimit, dailyRowsReadLimit, backfill, icon, relayName, writePolicy, writePolicySource, followCount, countAudit, followsListAt, vanishing, reads }`. `latestVersion` is the newest release of bothy, or null when the relay did not ask or got no answer, and `updateAvailable` says whether it is newer than `version`. `relayPubkey` is the relay's own signing key, the same value NIP-11 reports as `self`. `writePolicy` is the write policy in force by name and `writePolicySource` is where it was set (`env`, `stored` or `default`). `events24h` counts events by their own timestamp, which is what you posted; `ingested24h` counts what this relay actually took in, backfill included. During a backfill those differ by orders of magnitude. `backfill.status` is `pending`, `running`, `paused` (today's write budget is spent; it resumes at 00:00 UTC) or `done`. `countAudit` says when the daily count check last ran and lists any mismatch in plain sentences. `vanishing` is a count, a progress total and an age — never the pubkeys that asked, since this endpoint is public and naming them would publish exactly the list a vanish request exists to remove someone from. `reads` is how many rows the relay has read since it last woke, with a 24-hour projection.
...
- `GET /api/profile?pubkey=<npub or hex>` — the claim form's courtesy profile preview: it looks your kind-0 up on a couple of well-known relays so you can see the name and avatar attached to a pubkey before binding the relay to it permanently. **This is a setup endpoint and it is only reachable during setup** — once the relay is claimed it returns 404, because it exists to guard one irreversible step and there is no reason to leave a path that opens outbound connections to third-party relays permanently open to anybody. Results are cached for five minutes.
