...
- **Write policy** ([src/write-policy.ts](src/write-policy.ts)): one of five named levels, each including everything the one before it allows. `owner`: only the owner. `inbox`: plus kind-1059 gift wraps (NIP-59) from anyone, p-tag-addressed to the owner, under their own storage cap and per-IP throttle. `follows` (the default): plus the owner's kind-3 follow list, cached from the owner's stored contact list, refreshed when the owner publishes a new one here, with hourly cron as the fallback. `mentions`: plus any author whose event p-tags the owner, at most `MAX_MENTION_EVENT_INDEXED_TAGS` indexed tags per event from a non-follow. `all`: anyone, any event, bounded by the per-writer caps only; NIP-11 reports `restricted_writes: false` under it. Resolved from `WRITE_POLICY` in the environment, else the value stored by NIP-86 `changewritepolicy`, else the default; `relay.ts` caches the resolved policy per instance and clears it after every management call. `ownership.ts isAllowedWriter` enforces it, given the policy and the event's tags. The follow cache is maintained under every policy. NIP-86 `allowpubkey` admits and `banpubkey` refuses under every policy. `changewritepolicy all` refuses until called again with `OPEN_POLICY_CONFIRMATION` as its second parameter. NIP-29 invites enrol one named pubkey into that same allowlist.
...
- **NIP-29 groups are paused unless `GROUPS` is the exact string `"on"`** (`limits.ts groupsEnabled`). Paused: `nip29.ts authorizeGroupWrite` refuses every event in this relay's own group and every moderation kind with `GROUPS_PAUSED_MESSAGE`, the owner included; `relay.ts handleJoin` refuses a kind-9021 with the same message before the throttle and the signature check; the cron tick skips occupancy sampling and the chat sweep; NIP-11 does not list 29. Unchanged while paused: the partition and its read gate (stored group rows stay hidden from unauthenticated reads and readable by the owner and members), events tagged into another relay's group (still held to the member list), the NIP-86 invite and push methods, and the schema. The test bindings set `GROUPS=on`; `test/groups-paused.test.ts` covers the paused state with its own env.
...
- **It ships able to report and not to act.** `EPHEMERAL_CHAT` unset means REPORTING: each cron tick computes exactly what deleting mode would remove, logs it, and stores it in `chat_state.reported_at`/`reported_pending`, while nothing is deleted and nothing is withheld from a read — a relay in this mode behaves precisely as it did before. The exact string `"on"` lets it act; the exact string `"off"` removes the behaviour outright. That inverts this project's usual only-one-exact-string-disables-it shape (`UPDATE_CHECK`, the write-path caps) on purpose: those guard a safety limit, so turning one OFF is the act that must be spelled out, and here it is the deletion that must be. The numbers go to the log rather than to `/api/stats`, which is public and unauthenticated and which the group partition is kept off — the same channel `handleJoinRequest` uses for its refusal reasons. `/api/stats` carries `chatPolicy`, the mode and no counts, because a mode is configuration and says nothing about what the group holds.
...
`WRITE_POLICY` sets the write policy by name (`owner`, `inbox`, `follows`, `mentions`, `all`) and outranks the value stored through NIP-86 `changewritepolicy`; a malformed value is logged and ignored. Unset, the stored value applies, then the default, `follows`. `ALLOW_FOLLOWS` is no longer read.

`GROUPS`: the exact string `"on"` enables NIP-29 groups; anything else leaves them paused. See "What it is".
...
1. **Protocol conformance** — NIP-01 REQ/EVENT/CLOSE/EOSE, filters, replaceable/addressable/ephemeral storage rules, NIP-09/40/42/59/62, and NIP-86/98 management. Reject paths are asserted as carefully as accept paths. [test/write-policy.test.ts](test/write-policy.test.ts) covers the write policy level by level, the cumulative property, the `all` confirmation, and that a stored policy takes effect on the next event; [test/groups-paused.test.ts](test/groups-paused.test.ts) covers what a paused group refuses and what it leaves alone.
