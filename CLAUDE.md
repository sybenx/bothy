...
- **Writes are gated by the write ladder** ([src/write-policy.ts](src/write-policy.ts), [docs/rungs.md](docs/rungs.md)): one integer, 1–4, each rung CUMULATIVE over the one below it. Rung 1 is the owner alone; rung 2 adds kind-1059 gift wraps (NIP-59) from anyone, p-tag-addressed to the owner, gated by their own storage cap and per-IP throttle on top of the general write caps below; rung 3 (the default) adds the owner's kind-3 follow list (cached from the owner's own stored contact list, refreshed immediately when the owner publishes a new one to this relay, with hourly cron as the fallback for when it arrived some other way — never fetched per event); rung 4 adds any author whose event p-tags the owner, capped at `MAX_MENTION_EVENT_INDEXED_TAGS` indexed tags because "anyone" is a bound on people and not on rows. Rung 5, the open relay, does not exist in the code and is refused by name. The rung resolves through the identity chain's shape — `WRITE_RUNG` env var, then the legacy `ALLOW_FOLLOWS=false` (read as rung 2, which is what it always produced), then the value stored by NIP-86 `changewritepolicy`, then the default — and `relay.ts` caches the resolved rung per instance, invalidated after every management call, so the write path pays no row for it per event. `ownership.ts isAllowedWriter` takes the rung and the event's tags as context; the follow cache is maintained at every rung so raising the rung takes effect on the next event rather than the next cron tick. NIP-86 `allowpubkey` admits at every rung and `banpubkey` refuses at every rung: named individuals are what rung 1 already describes, and a ban is the owner closing the door on one person whatever the door's setting. NIP-29 invites are not a rung either: an invite enrols one named individual into that same allowlist — bounded by the owner's issuance rate and `MAX_OUTSTANDING_INVITES`, not by a new class of writer.
...
- **NIP-29 groups are PAUSED unless `GROUPS` is the exact string `"on"`** (`limits.ts groupsEnabled`). Paused: `nip29.ts authorizeGroupWrite` refuses every event in this relay's own group and every moderation kind with one message (`GROUPS_PAUSED_MESSAGE`), the owner included; `relay.ts handleJoin` refuses a kind-9021 with the same message before the throttle or the signature check, since the reason is configuration and not anything about the request; the cron tick neither samples occupancy nor runs the chat sweep; and NIP-11 does not list 29. What is NOT touched: the partition and its read gate (rows already in the group stay hidden from unauthenticated reads and readable by the owner and the members on the list — pausing a room is not emptying it), an event tagged into some OTHER relay's group (still held to the member list as before, because pausing this relay's group says nothing about foreign traffic), the NIP-86 invite and push methods (owner bookkeeping over what already exists), and the schema. The inverted switch shape is deliberate and matches `EPHEMERAL_CHAT`: enabling is the act that admits new writers to a partition, so enabling is the act that has to be spelled out. The test bindings set `GROUPS=on` so the group suites exercise the machinery; `test/groups-paused.test.ts` covers the paused state by passing its own env. README.md "What this is not" declined to claim group support before this switch existed; the switch makes the runtime match the README.
...
- **Group chat is ephemeral** ([src/storage.ts](src/storage.ts) `sweepChat`, the `limits.ts` block beginning `CONVERSATION_IDLE_SECONDS`). A conversation among friends is remembered rather than reread, and writing it down permanently turns talk into a record and changes what it was — the voice calls this relay carries have always worked that way and nobody expects to replay one, while the text beside them had been quietly claiming a different status nobody decided on. So kind-9 chat in the group expires: not hidden, not archived, deleted. Four rules. **A conversation ends when the room has been empty for `CONVERSATION_IDLE_SECONDS`** (two hours — long enough that dinner does not end it, short enough that an evening ends when the evening does), and everything said during it goes with it. **A message sent while somebody else was present belongs to that conversation.** **A message sent to an empty room is a note rather than speech**: it waits for the next conversation instead of expiring unseen, for as long as that takes — there is deliberately NO ceiling on a note, because speech is ephemeral for having been heard and a note has not been heard yet, and mail does not vanish because the person it was addressed to is away. The sender takes a note back the same way they take anything back, with a NIP-09 kind-5, which `applyDeletion` already honours across the group partition by primary key. **Somebody arriving partway through sees the last `CHAT_BACKLOG_SECONDS`** (five minutes), not the whole session — enough to know what is being talked about, not enough to replace asking.
- **The whole of that is two integers and an index seek**, which is the part worth defending. The obvious shape for "which conversation does this message belong to" is a per-message column or a side table, and both cost rows written per message to record something the clock already knows. `chat_state.last_occupied_at` is the last second at which `MIN_ROOM_OCCUPANTS` (two) distinct group readers held a socket at once, and it is simultaneously the thing that decides the conversation has ended and the line between what belonged to it and what did not: everything at or before the watermark is speech, everything after it is a note. So the sweep is `WHERE kind = 9 AND is_group = 1 AND created_at <= ?` against `idx_events_kind_created_grp`, which exists already and is partial over exactly this partition, and the speech/note rule needs no per-message bookkeeping at all. `chat_state.swept_through` is the second integer: it checkpoints a sweep too large for one cron tick, exactly as `vanishing.cutoff_created_at` does, and it is what `nip29.ts authorizeGroupWrite` compares an incoming chat message against so a member holding their own signed copy cannot replay a finished conversation back into the room.
- **Nothing is tombstoned, and that is the one place this departs from every other removal in the codebase.** A `deleted_ids` row per swept message would leave the relay accumulating two permanent rows for every message it deleted — a record of the conversation outliving the conversation, in a smaller box, which is the thing the feature exists to end. `swept_through` refuses the same replay for every message of every swept conversation at once, for one integer.
- **The relay is the only party that could do this**, which is why it is here and not in a client: it is the only one that sees everyone's presence at once, and a client expiring its own copy would be hiding a transcript this relay still held. `relay.ts groupOccupants` is that seeing — a walk over the object's own live sockets, counting distinct authenticated pubkeys that `handleReqInner`'s read gate already admitted to the group partition, so it costs no storage read at all. Sampled from three places: an accepted group write, a REQ by a group reader, and the cron tick, which is the only one that can see a room full of people sitting quietly. It is throttled as a whole rather than only at its write (`CHAT_OCCUPANCY_WRITE_INTERVAL_SECONDS`, 7.5 minutes, a sixteenth of the idle window) — throttling only the write left the watermark unmoved whenever the room held fewer than two people, so the throttle never advanced and every group event walked every socket to re-establish that nobody was there.
- **It ships able to report and not to act.** `EPHEMERAL_CHAT` unset means REPORTING: each cron tick computes exactly what deleting mode would remove, logs it, and stores it in `chat_state.reported_at`/`reported_pending`, while nothing is deleted and nothing is withheld from a read — a relay in this mode behaves precisely as it did before. The exact string `"on"` lets it act; the exact string `"off"` removes the behaviour outright. That inverts this project's usual only-one-exact-string-disables-it shape (`ALLOW_FOLLOWS`, `UPDATE_CHECK`, the write-path caps) on purpose: those guard a safety limit, so turning one OFF is the act that must be spelled out, and here it is the deletion that must be. The numbers go to the log rather than to `/api/stats`, which is public and unauthenticated and which the group partition is kept off — the same channel `handleJoinRequest` uses for its refusal reasons. `/api/stats` carries `chatPolicy`, the mode and no counts, because a mode is configuration and says nothing about what the group holds.
...
- **An expired gift wrap is deleted, not merely hidden** ([src/storage.ts](src/storage.ts) `sweepExpiredGiftWraps`, once per cron tick). NIP-40 expiry means *hide* for every other kind on this relay — `filters.ts` drops an expired event from every query, and the row then costs storage bytes that `NON_OWNER_STORAGE_SHARE_LIMIT` already bounds. Kind 1059 is the exception because it is the one kind capped by COUNT: `giftWrapCount` counts rows and not mail, so a hidden wrap goes on holding a slot in `maxGiftWraps` that no query will ever serve from. Left alone, a relay carrying expiring wraps fills its inbox with rows it is hiding and then answers `blocked: gift wrap inbox storage is full` to the owner's real mail — worse at the job it already had, in exchange for nothing. Teaching `giftWrapCount` about expiry instead is the smaller-looking fix and was rejected twice over: the count is answered from `idx_events_kind_created_*` without touching the table, and `expiration` is on no index, so it would fetch every row behind those index entries and double a read `handleGiftWrap` pays on every accepted wrap — and it would fix only half the problem, since `maxGiftWraps` is bounded by `MAX_FILTER_ROWS_READ` as well as by the storage share, and a dead row costs `excludeGiftWraps` the same skip a live one does. **No tombstone**, and the argument is stronger than the one the chat sweep makes for the same choice: `acceptEvent` already refuses any event whose `expiration` has passed, and the tag is covered by the signature, so the event refuses its own replay forever and for free — a `deleted_ids` row would be a permanent row bought to duplicate a check the event carries.
...
`EPHEMERAL_CHAT` is the one variable whose default is neither on nor off but *watch*. Unset, the relay reports each hour what it would delete and deletes nothing; `"on"` lets it act and clamps group chat reads to the horizon; `"off"` removes the behaviour entirely. See "What it is" above for why the shape is inverted from every other variable here, and "The budget" for what turning it on costs.

`WRITE_RUNG` pins the write ladder's rung (`1`–`4`, or `owner`/`inbox`/`follows`/`mentions`) from the environment, outranking the value stored through NIP-86 `changewritepolicy` the way `RELAY_NAME` outranks `changerelayname`; a malformed value is logged and ignored rather than read as any rung. Unset, the stored value applies, then the default of 3.

`ALLOW_FOLLOWS` is the legacy narrowing switch and is kept so existing deployments keep behaving: the literal string `"false"` reads as rung 2 (it never refused gift wraps, so rung 2 is what it always produced), and it sits below `WRITE_RUNG` in the chain. At rung 3 and above the owner's kind-3 follow list gates writes. The cache refreshes immediately when the owner publishes a new contact list to this relay; hourly cron is the fallback for when it arrived some other way (backfill, or a client that published it elsewhere first). The cache is maintained at every rung, not only where it is consulted — see "What it is".

`GROUPS` is the NIP-29 switch: the exact string `"on"` enables groups, anything else leaves them paused. See "What it is" for exactly what paused means.

`UPDATE_CHECK` defaults to on and takes the exact string `"off"` to disable, the same shape as the write-path caps. On, the Worker asks upstream what the current release is when `/api/stats` is served (`upstream-version.ts`, cached six hours per isolate) so the admin page can show a `v… available` note; off, it never makes that request and the note never appears. It is the read side of README.md "Keeping it updated", and it exists because the updater is opt-in by construction: nothing else in this project would ever tell an owner who did not commit `sync.yml` that a release happened.
...
[.github/workflows/sync.yml](.github/workflows/sync.yml) is how a downstream copy pulls in upstream changes — the deploy button clones this repo as an independent repo, not a fork, so there is no Sync fork button and nothing happens by itself. It is scheduled (weekly, `cron: "23 6 * * 1"`) as well as `workflow_dispatch`, and that is deliberate: committing the file IS enabling auto-update, because a switch that has to be found and clicked again every time is not one a button-deploy user will keep using. It no-ops in `sybenx/bothy` itself via the job-level `if` guard, so upstream's own Actions minutes are never spent on it.

The `git checkout HEAD -- wrangler.jsonc .github/` step is load-bearing — it restores the user's own Cloudflare resource IDs and this workflow after the upstream checkout overwrites them; don't remove or reorder it. The `.github/` half is not merely policy: `GITHUB_TOKEN` cannot push a change to a file under `.github/workflows/` (GitHub refuses the push outright), so a sync that tried to carry a newer workflow downstream would fail at the push rather than at the checkout. The consequence is that the workflow a downstream copy commits is the one it keeps forever — it has to be right when it ships, and a fix to it reaches existing deployments only if their owners re-commit it by hand.

The copy Cloudflare creates arrives without `.github/workflows/`, which is why README.md hands the file over as a GitHub "new file" URL carrying the whole workflow in its `?value=` parameter. That URL is generated, never hand-edited: `npm run sync-badge` rewrites it from the file, and `node scripts/sync-badge.mjs --check` exits non-zero when the two have drifted. Edit `sync.yml`, then run it — otherwise every new deployment enables a version of the updater that no longer exists here.
...
- **Unauthorized writes.** TOFU ownership binds one pubkey permanently; every
  event is signature-verified regardless of who sent it. The write ladder
  (`write-policy.ts`) widens writes one bounded class at a time — mail, the
  owner's kind-3 list, events that p-tag the owner — and stops there; there
  is no rung that admits an unbounded class. Rung 4's admission is decided
  from the event's own tags, and a stranger's event is capped in indexed
  tags so that admitting anyone bounds rows and not only authors. NIP-86
  `banpubkey` and `allowpubkey` adjust the set by hand, at every rung.
...
- [src/write-policy.ts](src/write-policy.ts) — the write ladder: the four rungs and their names, `parseRung` (which knows rung 5 by name so it can be refused with the reason), `resolveWriteRung` (env, then the legacy `ALLOW_FOLLOWS`, then stored, then default — handed the stored value rather than reading it, so it costs no rows of its own), the rung-4 mention test read off the event's `p` tags, and the indexed-tag cap that keeps rung 4 bounded in rows. Imported by `ownership.ts` for the gate, `relay.ts` for the cache and the gift wrap check, and `nip86.ts` for `changewritepolicy`/`getwritepolicy`.
- [src/ownership.ts](src/ownership.ts) — owner pubkey resolution, TOFU claim, follow-list cache, profile/icon refresh. `isAllowedWriter` takes a `WriterContext` (the resolved rung and the event's tags) and enforces the ladder; `refreshFollows` is not gated on the rung. `getOwnerPubkey` runs `OWNER_PUBKEY` through `normalizePubkey` like every other pubkey boundary in the project, memoised on the raw string because it sits on the write path. It did not, and returned the variable verbatim while every comparison target is lowercase hex — so an operator setting an npub (the form every client shows them) got a relay where the owner could not write, could not read their own gift wraps, and could not be addressed by one, silently. A malformed value now resolves to null, which reads as unclaimed and is visible; `index.ts` still gates `/api/claim` on the variable being *set*, so this fails closed rather than reopening TOFU.
...
- [src/upstream-version.ts](src/upstream-version.ts) — the "a newer bothy exists" check behind `/api/stats`'s `latestVersion`/`updateAvailable`, plus the isolate-local cache in front of it, the same shape and for the same `caches.default`-needs-a-custom-domain reason as `profile-lookup.ts`. Runs in the Worker and never in the Durable Object: an outbound request from there costs duration and blocks hibernation, and this is a courtesy notice on an unauthenticated read path. Reads `version` out of upstream's `package.json` on `main` — the same field the running relay reports about itself (CLAUDE.md "Release step"), so the two sides of the comparison cannot drift — over raw.githubusercontent rather than api.github.com, whose 60/hour unauthenticated limit is keyed to an outbound IP this Worker shares with every other Worker in its colo. Failures are cached too, on a shorter TTL, and answer null; null is "no answer", never "up to date", and every consumer renders it as no notice at all. Costs zero rows written and zero rows read — nothing about it is stored. `UPDATE_CHECK=off` turns it off outright.
...
- [scripts/sync-badge.mjs](scripts/sync-badge.mjs) — regenerates README.md's updater badge from `.github/workflows/sync.yml`, so the URL-encoded copy of the workflow in the README is derived rather than maintained. Outside `tsconfig`'s `include` and outside the deploy: a repo chore, not part of the Worker.
...
1. **Protocol conformance** — NIP-01 REQ/EVENT/CLOSE/EOSE, filters, replaceable/addressable/ephemeral storage rules, NIP-09/40/42/59/62, and NIP-86/98 management. Reject paths are asserted as carefully as accept paths. [test/write-policy.test.ts](test/write-policy.test.ts) covers the write ladder rung by rung, including the cumulative property (each rung admits everyone the one below it admits) and that a stored rung takes effect on the very next event; [test/groups-paused.test.ts](test/groups-paused.test.ts) covers what a paused group refuses and what it leaves alone.
2. **Budget/hibernation regression** — [test/hibernation.test.ts](test/hibernation.test.ts) asserts the object becomes eligible to hibernate after the last message, and pins the per-event rows-written cost against a real `SqlStorageCursor.rowsWritten`; [test/nip29-groups.test.ts](test/nip29-groups.test.ts) pins the per-membership-change cost the same way; [test/read-cost.test.ts](test/read-cost.test.ts) pins rows read per query shape; [test/push.test.ts](test/push.test.ts) pins that twelve presence heartbeats write zero rows on the presence path and that one fan-out spends no more than one invocation's subrequest allowance. That last file also decrypts a real push body back to its payload rather than asserting the encryption's shape — RFC 8291 has several places where a wrong constant produces a body that is well-formed and undecryptable, and every one of them looks like success from inside the relay. [test/ephemeral-chat.test.ts](test/ephemeral-chat.test.ts) pins what it costs to sweep a conversation, and pins the two figures that make the absence of a daily share defensible: five rows to remove a message against twelve to store it, and zero for a tick that finds nothing to do. [test/nip59-giftwrap.test.ts](test/nip59-giftwrap.test.ts) pins the same pair for the expired-gift-wrap sweep, plus the read cost that is peculiar to it — a tick with nothing to do walks the stored wraps once to establish that, and a tick with work stops when the batch fills. These assertions are the budget baseline — there is no separate file of recorded numbers to fall out of step with them.
